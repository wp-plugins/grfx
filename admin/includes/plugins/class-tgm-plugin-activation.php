    ...
    class TGM_Plugin_Activation {
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                if ( ! $this->dismissable && ! empty( $this->dismigrfx_msg ) ) {
                    $rendered .= '<p><strong>' . wp_kses_post( $this->dismigrfx_msg ) . '</strong></p>';
                }
                ...
                $action_links = apply_filters(
                    ...
                    array(
                        ...
                        'dismiss'  => $this->dismissable ? '<a class="dismigrfx-notice" href="' . add_query_arg( 'tgmpa-dismiss', 'dismigrfx_admin_notices' ) . '" target="_parent">' . $this->strings['dismiss'] . '</a>' : '',
                    )
                );
        ...
        public function config( $config ) {
            ...
            $keys = array( 'default_path', 'has_notices', 'dismissable', 'dismigrfx_msg', 'menu', 'is_automatic', 'message', 'strings' );
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function before( $title = '' ) {
                ...
                echo '<div class="update-messages hide-if-js" id="progregrfx-' . esc_attr( $this->upgrader->update_current ) . '"><p>';
            ...
            public function after( $title = '' ) {
                ...
                if ( $this->error || ! $this->result ) {
                    ...
                    echo '<script type="text/javascript">jQuery(\'#progregrfx-' . esc_js( $this->upgrader->update_current ) . '\').show();</script>';
                }

                // If the result is set and there are no errors, success!
                if ( ! empty( $this->result ) && ! is_wp_error( $this->result ) ) {
                    echo '<div class="updated"><p>' . sprintf( $this->upgrader->strings['skin_update_successful'], $this->plugin_names[$this->i], 'jQuery(\'#progregrfx-' . esc_js( $this->upgrader->update_current ) . '\').toggle();jQuery(\'span\', this).toggle(); return false;' ) . '</p></div>';
